...
class LoadNoduleGenerator():
    ...
    def nodule_predict(self, diff, num_images):
        ...
        """
        target_length = 140 * (diff ** 0.5)

        imgs = []
        
        # It is known that when the nodule generator is given a diff of 0.364 it will ouput nodules that have a side length of around 100

        for _ in range(num_images):
          with torch.no_grad():
            image = self.nodule_gen(torch.randn(1, self.channels_noise, 1, 1).to(self.device), 0.364)[0]
          ...
          new_length = int(140 * target_length/100)
          im = im.resize((new_length, new_length))

          if new_length > 140:
             border = (new_length - 140)//2
             img = im.crop((border, border, border + 140, border + 140))
          else:
             border = (140 - new_length)//2
             img = Image.new(mode=im.mode, size=(140, 140), color=background_color)
             Image.Image.paste(img, im, (border, border))
